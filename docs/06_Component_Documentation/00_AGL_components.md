---
title: AGL Components
---

# Components under development within AGL

## Services

### Graphics

- [The AGL compositor](01_Graphics_Service/01_agl_compositor.md)
- [DRM lease manager](01_Graphics_Service/02_drm_lease_manager.md)


### Sound

- [Pipewire & Wireplumber](02_Sound_Service/01_pipewire_wireplumber.md)
- [IC and Sound Manager](02_Sound_Service/02_Instrument_Cluster_Sound_Management.md)


### Policies

- [Rule based arbitrator](03_Policies_Service/01_Rule_Based_Arbitrator.md)


### Misc

- [Persistent storage](04_Misc_Service/01_AGL_Persistent_Storage_API.md)
- [Voice agent assistant](04_Misc_Service/02_agl_voice_agent_assistant.md)


## Instrument Cluster

- [Instrument Cluster](10_IC_Service/01_Instrument_Cluster_Service.md)


## IVI Application Framework

### Application Lifecycle and Services

- [Application Framework](20_IVI_Application_Framework/01_Introduction.md)


## Application for Demo

### Navigation

- [Momi Navi](40_Demo_Application/01_Momi_Navi.md)


## Platform extension

### Unified HMI

- [Unified HMI](60_Unified_HMI/01_Unified_HMI.md)

### Container

- [Container Manager](61_Container/01_Container_Manager.md)


## Development Tools

### Demo Control

- [Demo Control Panel](80_DevTools/01_AGL_Demo_Control_Panel.md)
- [CARLA with AGL](80_DevTools/02_CARLA_with_AGL.md)
